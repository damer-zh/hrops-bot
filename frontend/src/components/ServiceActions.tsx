...
export const ServiceActions: React.FC<ServiceActionsProps> = ({
    ...
    useEffect(() => {
        if (activeModal === "hr" && slots.length === 0) {
            api.get("/tma/appointments/slots")
    ...
    const requestCertificate = () =>
        handleAction(() =>
            api.post("/tma/certificate", {
                employeeId,
                type: parseInt(certType),
    ...
    const requestEquipment = () =>
        handleAction(() =>
            api.post("/tma/equipment", {
                employeeId,
                type: parseInt(equipType),
            }),
        );
    ...
    const searchRegulations = async () => {
        ...
        try {
            const res = await api.get(
                `/tma/regulations?q=${encodeURIComponent(regQuery)}`,
            );
